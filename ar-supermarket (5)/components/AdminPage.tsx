...
export default function AdminPage({ setProducts, setCoupons, setUpcomingDeals }) {
  ...
  const [products, setLocalProducts] = useState([
    { id: 1, name: 'Futuristic Cereal', price: 9.99, inStock: true, quantity: 50, image: '/placeholder.svg', model: '/models/cereal.glb' },
    { id: 2, name: 'Holo-Milk', price: 5.99, inStock: true, quantity: 100, image: '/placeholder.svg', model: '/models/milk.glb' },
    { id: 3, name: 'Quantum Coffee', price: 15.99, inStock: false, quantity: 0, image: '/placeholder.svg', model: '/models/coffee.glb' },
  ])
  ...
  const handleFileUpload = (e, type) => {
    ...
    if (file) {
      ...
      } else if (type === 'model' && file.size <= 20 * 1024 * 1024) { // 20MB limit for 3D models
    ...
    <div className="max-w-4xl mx-auto mt-10">
      ...
      <div className="mb-8">
        ...
        <form onSubmit={handleAddProduct} className="space-y-2">
          <input name="name" type="text" placeholder="Product Name" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <input name="price" type="number" step="0.01" placeholder="Price" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <input name="quantity" type="number" placeholder="Quantity in Stock" className="w-full p-2 rounded bg-gray-700 text-white" required />
          ...
          <div className="flex flex-col space-y-2">
            ...
            <input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, 'image')} className="text-sm text-gray-400" />
          </div>
          <div className="flex flex-col space-y-2">
            <label className="text-sm">3D Model (.obj or .fbx, max 20MB)</label>
            <input type="file" accept=".obj,.fbx" onChange={(e) => handleFileUpload(e, 'model')} className="text-sm text-gray-400" />
          </div>
      ...
      <div className="mb-8">
        ...
        <form onSubmit={handleAddCoupon} className="space-y-2">
          <input name="code" type="text" placeholder="Coupon Code" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <input name="discount" type="text" placeholder="Discount (e.g., 10% off)" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <input name="expiryDate" type="date" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <button type="submit" className="w-full bg-green-600 text-white p-2 rounded">Add Coupon</button>
        </form>
      </div>
      ...
      <div className="mb-8">
        ...
        <form onSubmit={handleAddDeal} className="space-y-2">
          <input name="name" type="text" placeholder="Deal Name" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <input name="discount" type="text" placeholder="Discount (e.g., 20% off)" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <input name="date" type="date" className="w-full p-2 rounded bg-gray-700 text-white" required />
          <button type="submit" className="w-full bg-green-600 text-white p-2 rounded">Add Deal</button>
        </form>
      </div>
