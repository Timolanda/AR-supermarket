...
export default function NeonMatcher({ onGameComplete, initialLoyaltyPoints, setActiveView }) {
  ...
  const calculatePoints = useCallback((finalScore) => {
  ...
  }, [gameState, currentLevel, score, generateNewWord, calculatePoints])
  ...
  const handleKeepPoints = () => {
    setLoyaltyPoints((prev) => prev + pointsGained)
    onGameComplete(loyaltyPoints + pointsGained)
  }
      ...
      <AnimatePresence mode="wait">
        {gameState === "start" && (
          ...
          >
            ...
            <p className="text-gray-300 mb-8">
              Match the word with its correct color to earn loyalty points! Complete all levels for maximum rewards.
            </p>
            <button
              onClick={startGame}
        ...
        {gameState === "wrong" && (
          <motion.div
            ...
            className="text-center space-y-6 p-8"
          >
            <h3 className="text-3xl font-bold text-red-500 mb-4">Oops! Wrong Answer</h3>
            <p className="text-xl text-gray-300">Would you like to continue or restart the game?</p>
            <div className="space-y-4 mt-8">
              <button
                onClick={() => setActiveView("shop")}
                className="px-8 py-4 bg-cyan-600 rounded-full text-white font-bold text-lg hover:bg-cyan-700 transition-all w-full"
              >
                Back to Shop
              </button>
              <button
                onClick={startGame}
                className="px-8 py-4 bg-blue-600 rounded-full text-white font-bold text-lg hover:bg-blue-700 transition-all w-full"
              >
                Restart Game
              </button>
            </div>
        ...
        {gameState === "end" && (
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-xl text-green-400">Total Loyalty Points: {loyaltyPoints + pointsGained}</p>
            </div>
